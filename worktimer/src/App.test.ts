...
describe("App", () => {
  it("adds a session via the Add session form", async () => {
    ...
    try {
      ...
      await fireEvent.input(getByLabelText(/^date$/i), {
        target: { value: "2026-06-22" },
      });
      await fireEvent.input(getByLabelText(/^session start$/i), {
        target: { value: "0900" },
      });
      await fireEvent.input(getByLabelText(/^session end$/i), {
        target: { value: "1230" },
      });
  ...
  it("rejects an added session that overlaps an existing one", async () => {
    ...
    try {
      ...
      expect(
        Array.from(alerts).some((a) => /overlap/i.test(a.textContent ?? "")),
      ).toBe(true);
    } finally {
      vi.useRealTimers();
    }
  ...
  it("renders a 24-week chart in the Analysis section", () => {
    ...
    try {
      ...
      expect(
        chart.querySelectorAll("rect[data-testid=chart-bar]"),
      ).toHaveLength(24);
      expect(
        chart.querySelectorAll("polyline[data-testid=chart-line]"),
      ).toHaveLength(1);
      const line = chart.querySelector("polyline[data-testid=chart-line]")!;
      const pointCount = (line.getAttribute("points") ?? "")
        .trim()
        .split(/\s+/).length;
      expect(pointCount).toBe(24);
      expect(chart.querySelectorAll("text[data-testid=x-tick]")).toHaveLength(
        24,
      );
      expect(
        chart.querySelectorAll("line[data-testid=gridline]").length,
  ...
  it("persists a WorkTargetsSet event on Save and updates the budget", async () => {
    ...
    try {
      ...
      await fireEvent.input(getByLabelText("Mo"), {
        target: { value: "0800" },
      });
      await fireEvent.input(getByLabelText("Tu"), {
        target: { value: "0800" },
      });
      await fireEvent.input(getByLabelText("We"), {
        target: { value: "0800" },
      });
      await fireEvent.input(getByLabelText("Th"), {
        target: { value: "0800" },
      });
      await fireEvent.input(getByLabelText("Fr"), {
        target: { value: "0800" },
      });
      await fireEvent.input(getByLabelText(/effective from/i), {
  ...
  it("deletes an adjustment when its Delete button is clicked", async () => {
    ...
    const deleteBtn =
      getByTestId("adjustment-history").querySelector("button")!;
    await fireEvent.click(deleteBtn);

    expect(loadEvents().filter((e) => e.type === "FlexAdjusted")).toHaveLength(
      0,
    );
    expect(getByTestId("flex-budget").textContent).toBe("+00:00");
  });
  ...
  it("creates a TargetOverride event via the override form", async () => {
    const { getByLabelText, getByRole } = render(App);
    await fireEvent.input(getByLabelText(/^from$/i), {
      target: { value: "2026-12-24" },
    });
    await fireEvent.input(getByLabelText(/^to$/i), {
      target: { value: "2026-12-26" },
    });
    await fireEvent.input(getByLabelText(/^target$/i), {
      target: { value: "0000" },
    });
    await fireEvent.input(getByLabelText(/override reason/i), {
  ...
  it("rejects an override where end is before start", async () => {
    ...
    expect(
      loadEvents().filter((e) => e.type === "TargetOverride"),
    ).toHaveLength(0);
    const alerts = document.querySelectorAll("[role=alert]");
    expect(
      Array.from(alerts).some((a) =>
        /end .* start|start .* end|range/i.test(a.textContent ?? ""),
      ),
    ).toBe(true);
  });
  ...
  it("lists overrides by start date (most recent first), regardless of emission order", async () => {
    localStorage.setItem(
      ...
      JSON.stringify([
        // Emitted second, but covers an earlier date.
        {
          ...
          id: "jan",
          at: 2000,
          startDay: new Date(2026, 0, 1).getTime(),
          endDay: new Date(2026, 0, 1).getTime(),
          targetMin: 0,
          reason: "new year",
        },
        // Emitted first, covers a later date.
        {
          ...
          id: "may",
          at: 1000,
          startDay: new Date(2026, 4, 1).getTime(),
          endDay: new Date(2026, 4, 3).getTime(),
          targetMin: 240,
          reason: "short days",
        },
      ]),
    ...
    await fireEvent.click(
      items[0].querySelector("button") as HTMLButtonElement,
    );
    expect(
      loadEvents().filter((e) => e.type === "TargetOverride"),
    ).toHaveLength(1);
  ...
  it("shows an error for invalid HHMM in the adjust form", async () => {
    ...
    const alerts = document.querySelectorAll("[role=alert]");
    expect(
      Array.from(alerts).some((a) => /hhmm/i.test(a.textContent ?? "")),
    ).toBe(true);
  });

  it("renders Today, This week, and Previous weeks sections", () => {
    const { getByText } = render(App);
    expect(getByText("Today", { selector: "h3" })).toBeTruthy();
  ...
  it("shows this week's total and per-past-day details", () => {
    ...
    try {
      ...
      const week = getByText("This week", { selector: "h2" }).closest(
        "section",
      )!;
      expect(getByTestId("week-total").textContent).toBe("10:00");

      const tueDetails = week.querySelector(
        `details[data-day-start="${tue}"]`,
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          { type: "WorkStarted", id: "a", at: lastTue + 9 * 3600_000 },
          {
            type: "WorkStopped",
            id: "b",
            at: lastTue + 13 * 3600_000 + 15 * 60_000,
          }, // 4h15m
        ]),
      );
      const { getByText } = render(App);
      const prev = getByText("Previous weeks", { selector: "h2" }).closest(
        "section",
      )!;
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          { type: "WorkStarted", id: "a", at: lastMon + 9 * 3600_000 },
          {
            type: "WorkStopped",
            id: "b",
            at: lastMon + 17 * 3600_000 + 37 * 60_000,
          }, // 8h37m
        ]),
      );
      const { getByText } = render(App);
  ...
  it("Previous weeks summary inserts month abbrev for day-of-month 01", () => {
    ...
    try {
      ...
      const matching = Array.from(
        prev.querySelectorAll("details summary"),
      ).find((s) => /W14/.test(s.textContent ?? ""))!;
  ...
  it("Previous weeks day numbers are colorised by delta and override", () => {
    ...
    try {
      ...
      const dayItems = Array.from(
        sum.querySelectorAll("[data-day-item]"),
      ) as HTMLElement[];
  ...
  it("does not prefix today's sessions with the weekday code", () => {
    ...
    try {
      ...
      const { getByText } = render(App);
      const todaySection = getByText("Today", { selector: "h3" }).closest(
        "section",
      )!;
  ...
  it("week-delta is not double-counted when today has stopped sessions and targets are set", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 5, 24, 12, 0, 0)); // Wed
    try {
      const tue = new Date(2026, 5, 23, 0, 0, 0).getTime();
      const today = new Date(2026, 5, 24, 0, 0, 0).getTime();
      const monStart = new Date(2026, 5, 22, 0, 0, 0).getTime();
      localStorage.setItem(
        "worktimer.events",
        JSON.stringify([
          { type: "WorkStarted", id: "tA", at: tue + 9 * 3600_000 },
          ...
          { type: "WorkStarted", id: "wA", at: today + 9 * 3600_000 },
          { type: "WorkStopped", id: "wB", at: today + 11 * 3600_000 }, // 2h today
          {
            type: "WorkTargetsSet",
            id: "tg",
            at: 1,
      ...
      const { getByTestId } = render(App);
      // 8h + 2h = 10h worked vs 40h week target → -30h (not -28h if today were double-counted)
      expect(getByTestId("week-delta").textContent).toBe("-30:00");
    } finally {
      vi.useRealTimers();
    }
  });

  it("week-total includes a running session tip on top of past-day sessions", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 5, 24, 11, 0, 0)); // Wed 11:00
    try {
      const tue = new Date(2026, 5, 23, 0, 0, 0).getTime();
      const today = new Date(2026, 5, 24, 0, 0, 0).getTime();
      localStorage.setItem(
        ...
        JSON.stringify([
          { type: "WorkStarted", id: "tA", at: tue + 9 * 3600_000 },
          { type: "WorkStopped", id: "tB", at: tue + 17 * 3600_000 }, // 8h Tue
          { type: "WorkStarted", id: "wA", at: today + 10 * 3600_000 }, // running since 10:00
        ]),
      );
      const { getByTestId } = render(App);
      // 8h past + 1h running tip = 9h
      expect(getByTestId("week-total").textContent).toBe("09:00");

      await vi.advanceTimersByTimeAsync(3600_000);
      await tick();
      // 8h past + 2h running tip = 10h
      expect(getByTestId("week-total").textContent).toBe("10:00");
    } finally {
      vi.useRealTimers();
    }
  ...
  it("shows weekly delta and per-day deltas when targets exist", () => {
    ...
    try {
      ...
      // Tue 8h worked vs 40h week target = -32h delta (week-total hidden when targets active)
      ...
      expect(tueDetails.querySelector("summary")!.textContent).toMatch(
        /[+\-]00:00/,
      );
    } finally {
      vi.useRealTimers();
  ...
  it("deletes a target set via its Delete button", async () => {
    ...
    const firstRowDelete = getByTestId("targets-history").querySelector(
      "li button",
  ...
  it("colours the start/stop button teal/crimson by state", async () => {
    const { getByRole } = render(App);
    expect(getByRole("button", { name: /start/i }).className).toContain(
      "btn-start",
    );
    await fireEvent.click(getByRole("button", { name: /start/i }));
    expect(getByRole("button", { name: /stop/i }).className).toContain(
      "btn-stop",
    );
  });
    ...
    localStorage.setItem(
      ...
      JSON.stringify([
        { type: "WorkStarted", id: "a", at: today + 9 * 3600_000 },
        {
          type: "WorkStopped",
          id: "b",
          at: today + 10 * 3600_000 + 30 * 60_000,
        },
      ]),
    );
    const { getAllByRole } = render(App);
  ...
  it("persists edited start/stop times when Save is clicked", async () => {
    ...
    const { getByRole, getByLabelText, queryByLabelText, getAllByRole } =
      render(App);
    await fireEvent.click(getByRole("button", { name: /edit/i }));

    await fireEvent.input(getByLabelText(/^start$/i), {
      target: { value: "0830" },
    });
    await fireEvent.input(getByLabelText(/^stop$/i), {
      target: { value: "1100" },
    });
  ...
  it("clears the error after a successful Save", async () => {
    ...
    const { getAllByRole, getByLabelText, getByRole, queryByRole } =
      render(App);
    const items = getAllByRole("listitem");
    await fireEvent.click(items[1].querySelector("button")!);
  ...
  it('populates events when "Load sample data" is clicked', async () => {
    ...
    await fireEvent.click(
      getByRole("button", { name: /replace current data with generated/i }),
    );
  ...
  it("replaces events when a JSON file is imported", async () => {
    ...
    const file = new File([JSON.stringify(incoming)], "events.json", {
